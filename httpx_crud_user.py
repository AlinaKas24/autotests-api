...
create_user = httpx.post(
    f"{settings.http_client.client_url}/api/v1/users", json=body_create
)
...
print("Get user me")
get_user = client.get(f"{settings.http_client.client_url}/api/v1/users/me")
...
get_user_by_id = client.get(f"{settings.http_client.client_url}/api/v1/users/{user_id}")
print(get_user_by_id.status_code)
print(get_user_by_id.json())

print("Update user")
update_user = client.patch(
    f"{settings.http_client.client_url}/api/v1/users/{user_id}", json=body_update
)
...
delete_user = client.delete(f"{settings.http_client.client_url}/api/v1/users/{user_id}")
